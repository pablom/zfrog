...
static void init_log( struct jsonrpc_log *log )
{
	log->msg = NULL;
	log->next = log;
	log->prev = log;
...
static void append_log(struct jsonrpc_log *prev, int lvl, char *msg)
{
	struct jsonrpc_log *new = mem_malloc(sizeof(struct jsonrpc_log));
...
static void free_log(struct jsonrpc_log *root)
{
...
void jsonrpc_log(struct jsonrpc_request *req, int lvl, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
    cf_buf_appendv(&req->buf, fmt, ap);
	va_end(ap);

    msg = cf_buf_stringify(&req->buf, NULL) + start;
...
static int read_json_body( struct http_request *http_req, struct jsonrpc_request *req )
{
    ...
    uint8_t chunk_buffer[BUFSIZ];
    char error_buffer[1024];

    for(;;)
    {
		chunk_len = http_body_read(http_req, chunk_buffer,sizeof(chunk_buffer));
        if( chunk_len == -1 )
        {
            jsonrpc_log(req, LOG_CRIT, "Failed to read request body");
        ...
        {
            jsonrpc_log(req, LOG_CRIT, "Request body bigger than the platform accepts");
            return JSONRPC_SERVER_ERROR;
		}

		body_len += chunk_len;

        cf_buf_append(&req->buf, chunk_buffer, chunk_len);
	}

    /* Grab our body data as a NUL-terminated string */
    body_string = cf_buf_stringify(&req->buf, NULL);

    /* Parse the body via yajl now */
	*error_buffer = 0;
    req->json = yajl_tree_parse(body_string, error_buffer, sizeof(error_buffer));

    if( req->json == NULL )
    {
        if( strlen(error_buffer) )
        {
            jsonrpc_log(req, LOG_ERR, "Invalid json: %s", error_buffer);
        }
        else
        {
...
static int parse_json_body( struct jsonrpc_request *req )
{
    ...
    if( proto == NULL )
    {
        jsonrpc_log(req, LOG_ERR, "JSON-RPC protocol MUST be indicated and \"2.0\"");
        return JSONRPC_PARSE_ERROR;
	}
    ...
    if( (req->id = yajl_tree_get(req->json, id_path, yajl_t_any)) != NULL )
    {
        if( YAJL_IS_NUMBER(req->id) )
        {
            ...
            {
                jsonrpc_log(req, LOG_ERR, "JSON-RPC id SHOULD NOT contain fractional parts");
                return JSONRPC_PARSE_ERROR;
			}
        }
        else if( !YAJL_IS_STRING(req->id) )
        {
            jsonrpc_log(req, LOG_ERR, "JSON-RPC id MUST contain a String or Number");
            return JSONRPC_PARSE_ERROR;
		}
	}

    /* Check method */
    if( (req->method = YAJL_GET_STRING(yajl_tree_get(req->json, method_path, yajl_t_string))) == NULL )
    {
        jsonrpc_log(req, LOG_ERR, "JSON-RPC method MUST exist and be a String");
        return JSONRPC_PARSE_ERROR;
	}

    /* Check params */
	req->params = yajl_tree_get(req->json, params_path, yajl_t_any);
    if( !(req->params == NULL || YAJL_IS_ARRAY(req->params) || YAJL_IS_OBJECT(req->params)) )
    {
        jsonrpc_log(req, LOG_ERR, "JSON-RPC params MUST be Object or Array");
        return JSONRPC_PARSE_ERROR;
	}
...
static int write_id(yajl_gen gen, yajl_val id)
{
    ...
    if( YAJL_IS_STRING(id) )
    {
        ...
        return yajl_gen_string(gen, (unsigned char *)id_str, strlen(id_str));
	}

    return yajl_gen_null( gen );
}

static int open_response( yajl_gen genctx, yajl_val id )
{
    int	status;

    if( YAJL_GEN_KO(status = yajl_gen_map_open(genctx)) ||
        YAJL_GEN_KO(status = YAJL_GEN_CONST_STRING(genctx, "jsonrpc")) ||
        YAJL_GEN_KO(status = YAJL_GEN_CONST_STRING(genctx, "2.0"))  ) {
        return status;
    }
...
static int close_response( yajl_gen genctx )
{
	int	status;
...
static int write_log( struct jsonrpc_request *req )
{
    bool wrote_smth = false;
	int	status = 0;
	
    for( struct jsonrpc_log *log = req->log.next; log != &req->log; log = log->next )
    {
        ...
        if( !wrote_smth )
        {
            if( YAJL_GEN_KO(status = YAJL_GEN_CONST_STRING(req->gen, "data")) ||
                YAJL_GEN_KO(status = yajl_gen_array_open(req->gen))    ) {
                return status;
            }
        ...
        if( YAJL_GEN_KO(status = yajl_gen_array_open(req->gen)) ||
            YAJL_GEN_KO(status = yajl_gen_integer(req->gen, log->lvl)) ||
            YAJL_GEN_KO(status = yajl_gen_string(req->gen, (unsigned char *)log->msg, strlen(log->msg))) ||
            YAJL_GEN_KO(status = yajl_gen_array_close(req->gen)) ) {
            return status;
        }
	}
...
static int write_error(struct jsonrpc_request *req, int code, const char *message)
{
    ...
    if( YAJL_GEN_KO(status = yajl_gen_string(req->gen, (const unsigned char *)message, strlen(message))) ) {
        return status;
    }
...
int jsonrpc_error(struct jsonrpc_request *req, int code, const char *msg)
{
    ...
    {
        cf_log(LOG_ERR, "jsonrpc_error: Failed to allocate yajl gen");
        http_response(req->http, 500, NULL, 0);
        jsonrpc_destroy_request(req);
        return CF_RESULT_OK;
	}

    yajl_gen_config(req->gen, yajl_gen_beautify, req->flags & yajl_gen_beautify);
    ...
    {
		size_t	start = req->buf.offset;
        cf_buf_appendf(&req->buf, "%d", code);
        msg_fallback = cf_buf_stringify(&req->buf, NULL) + start;
	}

    if( YAJL_GEN_KO(status = write_error(req, code, msg ? msg : msg_fallback)) )
    {
        cf_log(LOG_ERR, "jsonrpc_error: Failed to yajl gen text [%d]", status);
        http_response(req->http, 500, NULL, 0);
        jsonrpc_destroy_request(req);
        return CF_RESULT_OK;
	}
...
int jsonrpc_result(struct jsonrpc_request *req, int (*write_result)(struct jsonrpc_request *, void *), void *ctx)
{
    ...
    if( (req->gen = yajl_gen_alloc(NULL)) == NULL )
    {
        cf_log(LOG_ERR, "jsonrpc_result: Failed to allocate yajl gen");
    ...
    if( YAJL_GEN_KO(open_response(req->gen, req->id)) ||
            YAJL_GEN_KO(YAJL_GEN_CONST_STRING(req->gen, "result")) ||
            YAJL_GEN_KO(write_result(req, ctx)) ||
            YAJL_GEN_KO(yajl_gen_map_close(req->gen)) )
    {
        http_response(req->http, 500, NULL, 0);
        jsonrpc_destroy_request(req);
        return CF_RESULT_OK;
