...
struct tagnode {
    ...
    union
    {
        ...
        struct
        {
            char *varname, *dfltval;
            cf_tmpl_fmtfunc fmtfunc;
        }
        var;
...
struct template
{
    char *filename;        /* name of template file */
    char *tmplstr;         /* contents of template file */
    FILE *out;             /* template output file pointer */
    FILE *errout;          /* error output file pointer */
    tagnode *roottag;      /* root of parse tree */
    const cf_tmpl_fmtlist *fmtlist; /* list of format functions */
...
/*
 * cf_tmpl_fmtlist is a list of format functions, which are passed to
 * a template.  A TMPL_VAR tag can specify a format function for
 * outputting the variable with the fmt="fmtname" attribute.
 */
struct cf_tmpl_fmtlist
{
    cf_tmpl_fmtlist *next;   /* next list member */
    cf_tmpl_fmtfunc fmtfunc; /* pointer to format function */
    char name[1];         /* name of format function */
};

/*
 * variables are passed to a template in a tree consisting of
 * cf_tmpl_var, cf_tmpl_varlist and cf_tmpl_loop nodes.
 *
 * cf_tmpl_var is a simple variable (name and value)
 */
typedef struct cf_tmpl_var cf_tmpl_var;

struct cf_tmpl_var
{
    cf_tmpl_var *next;     /* next simple variable on list */
    const char *name;
    char value[1];      /* value and name stored here */
};

/*
 * cf_tmpl_varlist is a variable list of simple variables and/or
 * loop variables
 */

struct cf_tmpl_varlist
{
    cf_tmpl_varlist *next;  /* next variable list on a list */
    cf_tmpl_var   *var;     /* list of my simple variables */
    cf_tmpl_loop  *loop;    /* list of my loop variables */
    cf_tmpl_loop  *parent;  /* my parent loop variable (if any) */
};

/* cf_tmpl_loop is a loop variable, which is a list of variable lists */
struct cf_tmpl_loop
{
    cf_tmpl_loop    *next;    /* next loop variable on a list */
    char            *name;    /* my name */
    cf_tmpl_varlist *varlist; /* list of my variable lists */
    cf_tmpl_varlist *tail;    /* tail of "varlist" */
    cf_tmpl_varlist *parent;  /* my parent variable list */
};

static template * newtemplate( char *filename, char *tmplstr, const cf_tmpl_fmtlist *fmtlist, FILE *out, FILE *errout );
...
static char * valueof( const char *varname, const cf_tmpl_varlist *varlist );
static cf_tmpl_loop * findloop( const char *loopname, const cf_tmpl_varlist *varlist );
static int is_true( const tagnode *iftag, const cf_tmpl_varlist *varlist );
static void write_text(const char *p, int len, FILE *out);
static char * newfilename( const char *inclfile, const char *parentfile );
static void walk( template *t, tagnode *tag, const cf_tmpl_varlist *varlist );
...
static template * newtemplate( char *filename, char *tmplstr, const cf_tmpl_fmtlist *fmtlist, FILE *out, FILE *errout )
{
    ...
    if( tmplstr == 0 )
    {
        if( (fp = fopen(filename, "r")) != 0 &&
            ...
            (buf = (char *) mem_malloc(stb.st_size + 1)) != NULL &&
            (stb.st_size == 0 ||
            fread(buf, 1, (size_t)stb.st_size, fp) == (size_t)stb.st_size))
        {
            fclose(fp);
            buf[stb.st_size] = 0;
        }
    ...
    t = (template *) mem_malloc(sizeof(*t));
    t->filename = filename != NULL ? filename : NULL;
...
static tagnode * newtag( template *t, tag_kind kind )
{
    ...
    default:
        ret = (tagnode *) mem_malloc(sizeof(*ret));
        break;
    }
    ret->kind = kind;
    ret->next = 0;
...
static void freetag( tagnode *tag )
{
    ...
    case tag_var:
        mem_free( (void *) (tag->tag.var.varname) );
        if( tag->tag.var.dfltval != 0 )
            mem_free((void *) tag->tag.var.dfltval);
        break;

    case tag_if:
    case tag_elsif:
        mem_free( (void *) tag->tag.ifelse.varname );
        if( tag->tag.ifelse.testval != 0 )
            mem_free((void *) tag->tag.ifelse.testval);
    ...
    case tag_loop:
        mem_free((void *) tag->tag.loop.loopname);
        freetag(tag->tag.loop.body);
        break;

    case tag_include:
        mem_free((void *) tag->tag.include.filename);

        if( (t = tag->tag.include.tmpl) != 0 )
        {
            mem_free((void *) t->filename);
            mem_free((void *) t->tmplstr);
            freetag(t->roottag);
            mem_free(t);
        }
        break;
...
static tagnode * parselist( template *t, int stop )
{
    ...
    while( (tag = t->curtag) != 0 )
    {
        ...
        case tag_endloop:
            ...
            t->error = 1;
            scan(t);
            if( tag->kind == tag_elsif ) /* tag linked to list to be freed later */
                break;  
 ...
 * EXPORTED FUNCTIONS
 *
 * cf_tmpl_add_var() adds one or more simple variables to variable list
 ...
 * is a variable name and value that we store in a cf_tmpl_var struct and
 * link into "varlist".
 *******************************************************************/
cf_tmpl_varlist* cf_tmpl_add_var( cf_tmpl_varlist *varlist, ... )
{
    ...
    {
        /*
         * get enough memory to store the cf_tmpl_var struct and
         * the name string and the value string
         */
        nlen = strlen(name) + 1;
        vlen = strlen(value) + 1;
        var = (cf_tmpl_var *) mem_malloc(sizeof(*var) + nlen + vlen);
        strcpy(var->value, value);
        var->name = strcpy(var->value + vlen, name);
        
        if( varlist == 0 )
        {
            varlist = (cf_tmpl_varlist *) mem_malloc(sizeof(*varlist));
...
/*******************************************************************
 * cf_tmpl_add_loop() adds loop variable "loop" to variable list "varlist"
...
/*******************************************************************
 * cf_tmpl_add_varlist() adds variable list "varlist" to loop variable
...
cf_tmpl_loop* cf_tmpl_add_varlist(cf_tmpl_loop *loop, cf_tmpl_varlist *varlist)
{
    ...
    if( loop == 0 )
    {
        loop = (cf_tmpl_loop *) mem_malloc(sizeof(*loop));
...
/*******************************************************************
* cf_tmpl_free_varlist() recursively frees memory
* used by a cf_tmpl_varlist
*******************************************************************/
void cf_tmpl_free_varlist(cf_tmpl_varlist *varlist)
{
    cf_tmpl_loop *loop, *loopnext;
    cf_tmpl_var  *var,  *varnext;
    ...
    {
        loopnext = loop->next;
        cf_tmpl_free_varlist( loop->varlist );
        mem_free((void *) loop->name);
        mem_free(loop);
    }
    ...
    cf_tmpl_free_varlist(varlist->next);
    mem_free(varlist);
}
/*******************************************************************
 * cf_tmpl_add_fmt() adds a name and function pointer to format function
 * list "fmtlist" and returns the result.  If "fmtlist" is null, then
 * we create it, otherwise we return "fmtlist".  Parameter "name" is
 * the name used in the in the fmt="fmtname" attribute of a cf_tmpl_var
...
{
    cf_tmpl_fmtlist *newfmt = NULL;

    if( name == 0 || fmtfunc == 0 )
        return fmtlist;

    newfmt = (cf_tmpl_fmtlist *) mem_malloc(sizeof(*newfmt) + strlen(name));
...
void cf_tmpl_free_fmtlist( cf_tmpl_fmtlist *fmtlist )
{
    if( fmtlist != 0 )
    {
        cf_tmpl_free_fmtlist( fmtlist->next );
        mem_free(fmtlist);
    }
}
/*******************************************************************
...
int cf_tmpl_write( char *filename, char *tmplstr, const cf_tmpl_fmtlist *fmtlist, const cf_tmpl_varlist *varlist, FILE *out, FILE *errout)
{
    ...
    if( (t = newtemplate(filename, tmplstr, fmtlist, out, errout)) == NULL )
        return -1;
 ...
 *
 * cf_tmpl_encode_entity() converts HTML markup characters to entities
 *******************************************************************/
...
/*******************************************************************
 *  cf_tmpl_encode_url() does URL encoding (%xx)
 *******************************************************************/
