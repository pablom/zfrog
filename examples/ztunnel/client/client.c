...
int	ztunnel_peer_handle(struct peer *);
void ztunnel_peer_cleanup(struct peer *);
void ztunnel_connection_close(struct connection *);
void ztunnel_connection_cleanup(struct connection *);
void ztunnel_event_schedule(int, int, int, void *);

void ztunnel_set_nonblock(int);
int	ztunnel_write_local(struct peer *);
int	ztunnel_write_remote(struct peer *);
int	ztunnel_read_local(struct peer *, struct peer *);
int	ztunnel_read_remote(struct peer *, struct peer *);

void ztunnel_accept(struct peer *);
void ztunnel_bind(struct peer *, struct addrinfo *);
void ztunnel_connect(struct peer *, struct addrinfo *);
void ztunnel_peer_init(struct peer *, const char *, void (*cb)(struct peer *, struct addrinfo *));
void ztunnel_netbuf_create(struct netbuf **, struct netbuf_list *, uint8_t *, uint32_t);
...
{
	fprintf(stderr,
        "Usage: ztunnel-client [-h host] [-p path] "
...
int main(int argc, char *argv[])
{
	...
	nchanges = 0;
    events = calloc(ZQUEUE_EVENT_COUNT, sizeof(struct kevent));
    changelist = calloc(ZQUEUE_EVENT_COUNT, sizeof(struct kevent));
    if( events == NULL || changelist == NULL )
		fatal("calloc(): %s", errno_s);

	memset(&lpeer, 0, sizeof(lpeer));
    ztunnel_peer_init(&lpeer, argv[0], ktunnel_bind);
    ztunnel_event_schedule(lpeer.fd, EVFILT_READ, EV_ADD, &lpeer);
	...
	*(target_port)++ = '\0';
    ...
    for( ;; )
    {
        n = kevent(kfd, changelist, nchanges, events, ZQUEUE_EVENT_COUNT, NULL);
        if( n == -1 )
        {
            if( errno == EINTR )
				continue;
        ...
        for( i = 0; i < n; i++ )
        {
            ...
            if (events[i].flags & EV_EOF || events[i].flags & EV_ERROR )
            {
                ...
                ztunnel_connection_close(peer->connection);
				continue;
			}
            ...
            else
            {
                ...
                {
                    ztunnel_event_schedule(peer->fd, EVFILT_WRITE, EV_ADD | EV_ONESHOT, peer);
				}
			}
		}
...
void ztunnel_peer_init( struct peer *peer, const char *name, void (*cb)(struct peer *, struct addrinfo *) )
{
	int	r;
...
void ztunnel_accept(struct peer *peer)
{
    int	fd;
    struct connection *c;
    ...
    ztunnel_event_schedule(c->local.fd, EVFILT_READ, EV_ADD, &c->local);
    ztunnel_event_schedule(c->local.fd, EVFILT_WRITE, EV_ADD | EV_ONESHOT, &c->local);
    ...
    ztunnel_peer_init(&c->remote, remote_name, ktunnel_connect);
    ztunnel_netbuf_create(&c->local.recv_buf, NULL, NULL, NETBUF_RECV_MAX);

    ztunnel_set_nonblock(c->local.fd);
    ztunnel_set_nonblock(c->remote.fd);
...
void ztunnel_bind(struct peer *peer, struct addrinfo *ai)
{
    ...
    {
        fatal("Cannot bind to %s:%s: %s", peer->host, peer->port, errno_s);
	}
    ...
    ztunnel_netbuf_create(&peer->recv_buf, NULL, NULL, NETBUF_RECV_MAX);
}

void ztunnel_connect(struct peer *peer, struct addrinfo *ai)
{
    int	l;
    char *req;
    ...
    {
        fatal("Cannot connect to %s:%s: %s", peer->host, peer->port, errno_s);
	}
    ...
    if( !SSL_connect(peer->ssl) )
        fatal("Could not establish an SSL connection to %s: %s", peer->host, ssl_errno_s);

	/* Send custom HTTP command. */
    l = asprintf(&req, HTTP_REQUEST_FMT, http_path, target_host, target_port, http_hostname);
    if( l == -1 )
    ...
    ztunnel_event_schedule(peer->fd, EVFILT_READ, EV_ADD, peer);
    ztunnel_event_schedule(peer->fd, EVFILT_WRITE, EV_ADD | EV_ONESHOT, peer);

	printf("Connected over SSL to %s:%s\n", peer->host, peer->port);
}

int ztunnel_peer_handle(struct peer *peer)
{
    int	r;
...
void ztunnel_connection_close(struct connection *c)
{
    printf("ztunnel_connection_close(%p)\n", c);
...
void ztunnel_connection_cleanup(struct connection *c)
{
    ztunnel_peer_cleanup(&c->local);
    ztunnel_peer_cleanup(&c->remote);

	free(c);
}

void ztunnel_peer_cleanup(struct peer *peer)
{
    struct netbuf *nb, *next;

    printf("ztunnel_peer_cleanup(%p)\n", peer);
...
void ztunnel_netbuf_create( struct netbuf **out, struct netbuf_list *head, uint8_t *data, uint32_t length )
{
    struct netbuf *nb;
...
void ztunnel_event_schedule(int fd, int type, int flags, void *udata)
{
    if( nchanges >= ZQUEUE_EVENT_COUNT )
        fatal("nchanges > ZQUEUE_EVENT_COUNT");

	EV_SET(&changelist[nchanges], fd, type, flags, 0, 0, udata);
	nchanges++;
}

int ztunnel_read_local(struct peer *in, struct peer *out)
{
    int	r;

    printf("ztunnel_read_local: %p\n", in);
    ...
    printf("ztunnel_read_local: %p -- %d bytes --> %p\n", in, r, out);

    ztunnel_netbuf_create(NULL, &(out->write_queue), in->recv_buf->data, r);
    return ztunnel_write_remote(out);
}

int ztunnel_write_local(struct peer *peer)
{
    int	r;
    struct netbuf *nb;
    ...
    while( !TAILQ_EMPTY(&peer->write_queue) )
    {
        ...
        printf("ztunnel_write_local: %p writing %d/%d\n", peer, nb->offset, nb->length);

        r = write(peer->fd, (nb->data + nb->offset), (nb->length - nb->offset));
		...
		nb->offset += r;
        printf("ztunnel_write_local: %p progress %d/%d\n", peer, nb->offset, nb->length);
...
int ztunnel_read_remote( struct peer *in, struct peer *out )
{
    int	r;
    ...
    ztunnel_netbuf_create(NULL, &(out->write_queue), in->recv_buf->data, r);
    return ztunnel_write_local(out);
}

int ztunnel_write_remote(struct peer *peer )
{
    int	r;
    struct netbuf *nb;
    ...
    while( !TAILQ_EMPTY(&peer->write_queue) )
    {
        ...
        printf("ztunnel_write_remote: %p writing %d/%d bytes\n", peer, nb->offset, nb->length);

		r = SSL_write(peer->ssl, (nb->data + nb->offset),(nb->length - nb->offset));
        if( r <= 0 )
        {
		...
		nb->offset += r;
        printf("ztunnel_write_remote: %p progress %d/%d\n", peer, nb->offset, nb->length);
...
void ztunnel_set_nonblock(int fd)
{
    int	flags;
