...
int v1( struct http_request *http_req )
{
	...
	if( strcmp(req.method, "echo") == 0 ) 
	{
		...
		{
			jsonrpc_log(&req, LOG_ERR, "Echo only accepts positional params");
		...
		for( size_t i = 0; i < req.params->u.array.len; i++ ) 
		{
			...
			if( !YAJL_IS_STRING(v) ) 
			{
				jsonrpc_log(&req, -3, "Echo only accepts strings");
				return jsonrpc_error(&req, JSONRPC_INVALID_PARAMS, NULL);
			}
		}
	...
	if( strcmp(req.method, "date") == 0 ) 
	{
		...
		{
			jsonrpc_log(&req, LOG_ERR, "Date only accepts named params");
			return jsonrpc_error(&req, JSONRPC_INVALID_PARAMS, NULL);
		}

		if( (time_value = time(NULL)) == -1 )
			return jsonrpc_error(&req, -2, "Failed to get date time");
		
		if( gettm(&time_value, &time_info) == NULL )
			return jsonrpc_error(&req, -3, "Failed to get date time info");
		
		memset(timestamp, 0, sizeof(timestamp));
		if( strftime_l(timestamp, sizeof(timestamp) - 1, "%c", &time_info, LC_GLOBAL_LOCALE) == 0 )
			return jsonrpc_error(&req, -4, "Failed to get printable date time");
