...
int subscribe( struct http_request *req )
{
	...
	{
		cf_log(LOG_ERR, "%p: already subscribed", req->owner);
	...
	state->timer = cf_timer_add(sse_ping, 10000, req->owner, 0);

	/* Respond that the SSE channel is now open */
	cf_log(LOG_NOTICE, "%p: connected for SSE", req->owner);
...
void sse_ping(void *arg, uint64_t now)
{
